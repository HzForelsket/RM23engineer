...
private:
    ...
    template<typename T>
    T setInRange(T data, T max, T min);//…Ë÷√ ˝æ›∑∂Œß
public:
    ...
    MOTOR_2006(PID angle_to_speed, PID speed_to_electricity, ID id, CAN* can,
        int16_t angleMax = 8192, int16_t angleMin = 0, int16_t electricityMax = 16384,
        int16_t speedMAx = 2000);
};
template<typename T>
T MOTOR_2006::setInRange(T data, T max, T min)
{
...
template<typename T>
T MOTOR_2006::getAngleDelta(T now, T pre, T direct)
{
    T temp = now - pre;
    if (temp <= -4096)
    {

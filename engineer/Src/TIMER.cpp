...
TIMER& TIMER::initBase(TIMER_MODE mode, TIM_TypeDef* TIMERx, double frequency, uint8_t openInterrupt, double prescaler)
{
    ...
    uint32_t APB1_clock = rcc_clock.PCLK1_Frequency ;
    uint32_t APB2_clock = rcc_clock.PCLK2_Frequency ;
    ...
    if (TIMERx == TIM1 || TIMERx == TIM8)
    {
        ...
        if ((double)APB2_clock / (double)count > 1.0)
            prescaler = (double)APB2_clock / (double)count / frequency - 1;
        else
        {
            prescaler = 0;
            count = (double)APB2_clock / (double)frequency;
        }
    ...
    {
        uint32_t count = 0x10000;
        if ((double)APB1_clock / (double)count > 1.0)
            prescaler = (double)APB1_clock / (double)count / frequency - 1;
        else
        {
            prescaler = 0;
            count = (double)APB1_clock / (double)frequency;
        }
